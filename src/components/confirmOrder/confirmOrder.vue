	...
	<div class="order-wrap">
		...
		<ul class="mark-wrap">
			...
			<li class="clearfix" id="store">
				<img v-if="image.pic_url" :src="URL + image.pic_url " class="fl">
				<img v-else src="http://center.shopsn.cn/Uploads/goods/2019-08-16/5d564e9785339.jpg" class="fl">
		...
		<div class="price-set-wrap">
			...
			<div class="freight price clearfix" v-if="money">
				<span class="fl">配送</span>
				<span class="fr">+&nbsp;&nbsp;<i>￥{{money}}</i></span>
			</div>
	...
	export default {
		...
		// 提交订单 单个商品购买
			toCashierAll() {
				...
				if(money) {
					let total = parseFloat(this.goods_data.price_member * this.goods_data.goods_num) + parseFloat(this.money) - parseFloat(this.$store.state.const_coupon_price);
				}
				else {
